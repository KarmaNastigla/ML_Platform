...
class UniversalMLEngine:
    def __init__(self, model_type="Random Forest"):
        ...
        # Артефакты, сохраняемые после обучения
        self.y_test      = None
        self.y_pred      = None
        self.class_labels = None
        self.conf_matrix  = None
        self.cv_scores    = None      # массив CV-метрик (None если CV не использовался)
        self.cv_mean      = None
        self.cv_std       = None
    ...
    def train_and_evaluate(
        ...
        """
        use_cv=True  — Optuna оценивает каждую конфигурацию через K-Fold CV
                       (честнее, но дольше). Финальная оценка тоже по CV.
        use_cv=False — быстрый режим: один train/test split (0.8/0.2).
        """
        n_before = len(df)
        ...
        # Обычный hold-out сплит нужен в любом случае — для confusion matrix и y_pred
        ...
        # ------------------------------------------------------------------
        # Подбор гиперпараметров / Ансамбль
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # Финальное обучение на всём X_train и оценка
        # ------------------------------------------------------------------
    ...
    def compute_shap_values(self, row_df: pd.DataFrame):
        """
        Возвращает (shap_vals_1d, base_value, feature_names) или None при ошибке.
        Поддерживает: RF, GB, LogisticRegression, Ridge, VotingClassifier/Regressor.
        """
        try:
            import shap
        except ImportError:
        ...
        try:
            ...
            elif sv_arr.ndim == 3:          # (1, F, C) — RF новый API
                sv_arr    = sv_arr[0, :, -1]
                ev_scalar = float(ev_arr[-1])
